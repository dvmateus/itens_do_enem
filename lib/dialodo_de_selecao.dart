...
class _DialogoSelecaoState extends State<DialogoSelecao> {
  ...
  void _removerFiltro({String opcao}){
    if (opcao == null) _tempFiltros.keys.forEach((_) => _tempFiltros[_] = null);
    else _tempFiltros[opcao] = null;
  ...
  Widget build(BuildContext context) {
    return SimpleDialog(
      ...
      children: [Column(children: [
        ...
        _gerarListTile(COL_COR_CADERNO),
        ...
        Wrap(
          ...
          children: [
            ...
            MaterialButton(onPressed: () => Navigator.pop(context), child: Text("Cancelar")),
            StreamBuilder(
              stream: streamFiltros,
              initialData: _tempFiltros,
              builder: (context, snapshot){
                final Map<String, dynamic> map = snapshot.data;
                bool ativar = false;
                map.keys.forEach((_) => ativar = ativar || _tempFiltros[_] != filtrosSelecionados[_]);
                return MaterialButton(
                  onPressed: !ativar
                    ? null
                    : () {
                    if (dados.filtrar(_tempFiltros) == 0) {
                      showDialog(
                        context: context,
                        child: AlertDialog(
                          title: const Text("Não há itens com essas informações"),
                          content: const Text("Altere os parâmetros de filtro"),
  ...
  Widget _gerarListTile(String opcao){
    ...
    switch (opcao){
      case COL_ANO:
        ...
        opcoesEscolha = anos.toList();
        break;
      case COL_COR_CADERNO:
        titulo1 = "Cor do caderno";
        titulo2 = "Escolha a cor do caderno";
        opcoesEscolha = coresCadernos.toList();
        break;
    ...
    return ListTile(
      ...
      trailing: StreamBuilder(
        ...
        builder: (context, snapshot){
          if (_tempFiltros[opcao] == null) return SizedBox();
          else return InputChip(
            label: Text(
              opcao == COL_ID_HABILIDADE
  ...
  Future _dialogoEscolher(String titulo, List opcoesEscolha, opcao){
    ...
    return showDialog(
      ...
      child: SimpleDialog(
        title: Text(titulo),
        children: List.generate(opcoesEscolha.length, (index) {
          final opcaoEscolhida = opcoesEscolha[index];
          return SimpleDialogOption(
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text(opcao == COL_ID_HABILIDADE ? "H${opcaoEscolhida}" : opcaoEscolhida.toString()),
                Icon(
                  Icons.check,
